...
- Standalone self-containment fix (2026-08-02): `scripts/generate_figures.py` previously imported the monorepo-only `infrastructure.documentation.generated_figure_registry`, so a detached standalone clone of this exemplar failed 5 integration tests. Added the byte-compatible project-local publisher `src/data_descriptor/registry.py` and a fallback import in the script (monorepo keeps the shared publisher; standalone clones use the local one). A fresh standalone clone now passes 54/54 tests and regenerates all five figures plus the registry with no `infrastructure` dependency.
